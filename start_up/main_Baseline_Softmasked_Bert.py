...
from dataset.sighan13_dataset_softmasked_bert import Sighan13
from model.baseline_softmasked_bert import SoftMaskedBert
...
if __name__== "__main__":
    ...
    # dataset
    train_dataset=Sighan13(train_errors, train_corrects)
    test_dataset=Sighan13(test_errors, test_corrects)
    
    
    # model and training setting
    CFG.model_name="Baseline_Softmasked_Bert"
    CFG.bert_name="bert-base-chinese"
    # # bert, tokenizer, hidden, layer_n, device
    CFG.hidden=768
    CFG.layer_n=1
    model=SoftMaskedBert(CFG).to(CFG.device)
